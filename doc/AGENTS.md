...
Do not use playful, whimsical, cute, decorative, or filler progress phrases. Be succint, accurate, intelligent, precise.
...
9. Update `STATUS.md` if the project state changed. If necessary compress past updates so that the file does not grow arbitrarily. 
...
- `doc/`: user-facing and maintainer-facing documentation, when present.
