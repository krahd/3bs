...
if(error OR NOT schema EQUAL 2)
    message(FATAL_ERROR "Factory preset schemaVersion must be 2: ${error}")
endif()
...
foreach(index RANGE 0 ${last})
    ...
    foreach(field palette trailSeconds trailWidth extrusion bloom stars)
        string(JSON value ERROR_VARIABLE field_error GET "${json}" presets ${index} visual ${field})
        if(field_error)
            message(FATAL_ERROR "Preset ${index} visual block lacks ${field}: ${field_error}")
        endif()
    endforeach()
    foreach(field focus distance yaw pitch autoOrbit)
        string(JSON value ERROR_VARIABLE field_error GET "${json}" presets ${index} camera ${field})
        if(field_error)
            message(FATAL_ERROR "Preset ${index} camera block lacks ${field}: ${field_error}")
        endif()
    endforeach()
endforeach()
